...
# --- CONEXIÓN SEGURA ---
try:
    # Usamos la inyección directa de secretos para evitar el ConnectionRefusedError
    conn = st.connection(
        "supabase", 
...
# ==========================================
    ...
    if res_e.data:
        st.table(pd.DataFrame(res_e.data)[['nombre', 'rol']])
...
# ==========================================
    ...
    st.button("⬅️ VOLVER", on_click=ir_a, args=('Home',))
    st.header("🔥 Control de Stock y Ventas")
    ...
    stock_ayer = res_ayer.data[0]['resto'] if res_ayer.data else 0
    
    c1, c2, c3, c4 = st.columns(4)
    st_ini = c1.number_input("Stock Inicial Real:", value=int(stock_ayer))
    horn = c2.number_input("Horneados:", min_value=0)
    merm = c3.number_input("Merma:", min_value=0)
    rest = c4.number_input("Resto (Cierre):", min_value=0)

    # Ventas automáticas
    desviacion = st_ini - stock_ayer if st_ini < stock_ayer else 0
    ventas = st_ini + horn - merm - rest
    
    st.info(f"💡 Ventas calculadas para {prod_sel}: **{ventas} unidades**")
    if desviacion < 0:
        st.error(f"📉 Desviación de stock detectada: {desviacion} unidades.")

    if st.button("💾 GRABAR Y LIMPIAR"):
        datos = {
...
# ==========================================
    ...
    if archivo:
        ...
        if st.button("🚀 SUBIDA MASIVA"):
            # Lógica de subida por lotes que ya usamos
            st.write("Procesando...")

# [Pantallas Dashboard y Productos se mantienen con el código anterior]
